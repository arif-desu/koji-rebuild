...
class KojiSession(koji.ClientSession):
    ...
    def __init__(self, instance: dict):
        ...
        if not any(config):
            sys.stderr.write(
                "Error parsing the configuration file. Check logs for possible issues."
            )
            sys.exit(1)
        ...
        if self.auth is not None:
            if self.auth == "ssl":
                try:
                    ...
                    self.logger.warning(
                        "SSL certificate info not defined in %s" % configfile,
                        exc_info=True,
                    )
                    self.set = False
                ...
                try:
                    ...
                    self.logger.warning(
                        "Kerberos authentication info not defined in %s" % configfile
                    )
                    self.set = False
                except (FileNotFoundError, PermissionError) as e:
                    self.logger.warning(e)
            ...
            self.logger.warning(
                'Unsupported authentication method "%s" specified in configuration file'
                % self.auth
            )
            self.set = False
    ...
    def auth_login(self) -> bool:
        ...
        if self.set is not False:
            if self.auth == "ssl":
                info = self.ssl_login(cert=self._client_cert, serverca=self._ca_cert)
            elif self.auth == "kerberos":
                info = self.gssapi_login(principal=self._principal, keytab=self._keytab)
            else:
